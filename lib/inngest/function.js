      ...
      await step.run("save-transactions", async () => {
        ...
        await db.$transaction(async (tx) => {
          const formattedTransactions = transactions
            .filter(
              (t) => t.amount != null && t.type != null && Number(t.amount) > 0,
            )
            .map((t) => ({
              amount: Number(t.amount),
              date: parseDate(t.date),
              description:
                t.description || t.merchantName || "Imported transaction",
...
export const processReceiptScan = inngest.createFunction(
  {
    id: "process-receipt-scan",
    name: "Process Receipt Scan",
  ...
  { event: "receipt/scan" },
  async ({ event, step }) => {
    const { base64Image, mimeType, accountId, userId } = event.data;

    const scannedData = await step.run("scan-receipt-with-gemini", async () => {
      const model = genAI.getGenerativeModel({
        model: "gemini-flash-latest",
      });
...
`;
    ...
    if (!scannedData || !scannedData.amount) {
      await db.notification.create({
        data: {
          ...
          message:
            "We couldn't read the receipt. Please add the transaction manually.",
          type: "ALERT",
        },
      });
    ...
    await step.run("save-transaction", async () => {
      ...
      const amount = Number(scannedData.amount);
      const date = new Date(scannedData.date);
      const monthString = date.toISOString().slice(0, 7);
      const newBalance = account.balance.toNumber() - amount; // receipts are always EXPENSE
      ...
      await db.$transaction(async (tx) => {
        await tx.transaction.create({
          data: {
            ...
            date,
            description:
              scannedData.description || scannedData.merchantName || "Receipt",
            category: scannedData.category || "other-expense",
            userId,
            accountId,
          },
        });
        ...
        await tx.account.update({
          where: { id: accountId },
          data: { balance: { increment: -amount } }, // ✅ relative increment, not absolute
        });

        await tx.budget.upsert({
          where: { userId_month: { userId, month: monthString } },
          update: { spent: { increment: amount } }, // ✅ this was missing
          create: { userId, month: monthString, amount: 0, spent: amount },
        });
        ...
        await tx.notification.create({
          data: {
            ...
            message: `₹${amount} from ${scannedData.merchantName || "merchant"} added automatically.`,
            type: "IMPORT",
          },
        });
      });
